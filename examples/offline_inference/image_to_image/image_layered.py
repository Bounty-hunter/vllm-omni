...
"""
Example script for image editing with Qwen-Image-Layered.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Edit an image with Qwen-Image-Layered.")
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        ...
        type=float,
        default=4.0,
        help="True classifier-free guidance scale specific to Qwen-Image-Layered.",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="./image_layers/",
        help="Path to save the image layers.",
    )
...
def main():
    ...
    # Load input image
    input_image = Image.open(args.image).convert("RGBA")
    ...
    # Initialize Omni with QwenImageLayeredPipeline
    omni = Omni(
        model=args.model,
        model_class_name="QwenImageLayeredPipeline",
    ...
    for idx, output in enumerate(images):
        for layer, image in enumerate(output):
            image_name = f"output_{idx}_layer_{layer}.png"
            save_path = output_path.parent / image_name
            image.save(save_path)
            print(f"Saved layered image to {os.path.abspath(save_path)}")
