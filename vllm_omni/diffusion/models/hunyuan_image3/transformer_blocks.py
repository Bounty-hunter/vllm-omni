...
The small ``conv_nd``/``normalization``/``linear``/``zero_module`` helpers are
duplicated from ``hunyuan_image3_transformer.py`` on purpose: importing them
from there would make that module and this one import each other.
"""
