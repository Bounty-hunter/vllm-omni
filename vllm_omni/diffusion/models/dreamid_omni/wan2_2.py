...
@amp.autocast("cuda", enabled=False)
def rope_apply_1d(x, grid_sizes, freqs, ref_lengths=None, offset_base=100, freqs_scaling=None):
    ...
    # loop over samples
    output = []
    for i, (len,) in enumerate(grid_sizes.tolist()):
        seq_len = len
        assert len <= 1024, "RoPE dimensions cannot exceed 1024"

        # precompute multipliers
        x_i = torch.view_as_complex(x[i, :seq_len].to(torch.float32).reshape(seq_len, n, -1, 2))  # [len n d//2]
...
@amp.autocast("cuda", enabled=False)
def rope_apply_3d(
    x, grid_sizes, freqs, ref_lengths=None, offset_base=100
):  # x torch.Size([1, 24570, 12, 128])  grid_sizes tensor([[21, 30, 39]])  freqs torch.Size([1024, 64])
