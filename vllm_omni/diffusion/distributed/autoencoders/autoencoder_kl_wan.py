...
class DistributedAutoencoderKLWan(AutoencoderKLWan, DistributedVaeDecode):
    @classmethod
    ...
    def tile_merge_func(
        ...
        for i in range(grid_h):
            ...
            for j in range(grid_w):
                ...
                if i > 0:
                    tile = self.blend_v(coord_tensor_map[(i - 1, j)], tile, self.blend_height)
                if j > 0:
                    tile = self.blend_h(coord_tensor_map[(i, j - 1)], tile, self.blend_width)
                result_row.append(tile[:, :, :, : self.tile_sample_stride_height, : self.tile_sample_stride_width])
            result_rows.append(torch.cat(result_row, dim=-1))

        dec = torch.cat(result_rows, dim=3)[:, :, :, : self.sample_height, : self.sample_width]
