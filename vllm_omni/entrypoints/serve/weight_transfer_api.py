...
Mirrors the upstream vLLM RLHF dev API (``vllm/entrypoints/serve/dev/rlhf``)
but drives AsyncOmni's multi-stage orchestrator. The router is mounted only
when ``weight_transfer_config`` is enabled.

Payloads are backend-specific and are built by the trainer-side engine
(``WeightTransferTrainerFactory`` + ``HTTPVLLMWeightSyncClient`` in upstream
vLLM); callers should not hand-craft them. For the IPC backend the init
handshake is ``{"init_info": {"packed": false}}`` and update payloads carry
CUDA IPC handles (see ``IPCWeightTransferUpdateInfo``).
"""
...
async def init_weight_transfer_engine(request: Request):
    ...
    Request body: ``{"init_info": <backend-specific handshake>}``, e.g.
    ``{"init_info": {"packed": false}}`` for the IPC backend (the payload
    ``IPCTrainerWeightTransferEngine.trainer_init`` ships).
    """
    try:
...
async def update_weights(request: Request):
    """Send one weight update chunk to all configured stages.

    Request body: ``{"update_info": <backend-specific payload>}``. The payload
    is produced by the trainer-side engine (e.g. ``IPCWeightTransferUpdateInfo``
    fields for IPC: names, dtype_names, shapes, ipc_handles) — see upstream
    ``HTTPVLLMWeightSyncClient`` for the reference client.
    """
    try:
        body = await request.json()
    except json.JSONDecodeError as e:
